...
ISR(TIMER1_OVF_vect)
{
  ...
  case (1):
    ...
    break;
  case (2):
    (count1 >= pulse_ms) ? (digitalWrite(PULSE1, LOW), digitalWrite(LED, LOW), count1 = 0, pulse1_stp++) : (0);
    break;
  case (3):
    (count1 >= low1) ? (pulse1_stp = 1) : (0);
    break;
  }
}
...
{
  (dist >= 0 && dist < L1) ? (level = 1) : (dist >= L1 && dist < L2) ? (level = 2)
